package com.ahmedtikiwa.sunshine.app.sync;
...
import com.ahmedtikiwa.sunshine.app.BuildConfig;
import com.ahmedtikiwa.sunshine.app.DetailActivity;
...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    // an id you create that is matched to your notification so that you can reuse it
    ...
    private String[] getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if (cVVector.size() > 0) {
                ...
                // remove the weather data from the previous date so as to not blot the database
                // with old data
    ...
    long addLocation(String locationSetting, String cityName, double lat, double lon) {
        ...
        if (locationCursor.moveToFirst()) {
            int locationColumnIndex = locationCursor.getColumnIndex(WeatherContract.LocationEntry._ID);
            locationId = locationCursor.getLong(locationColumnIndex);
        } else {
        ...
        if (displayNotifications) {
            ...
            if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS) {
                ...
                if (cursor.moveToFirst()) {
                    ...
                    Intent notificationDetailIntent = new Intent(getContext(), DetailActivity.class).setData(weatherUri);
                    ...
                    stackBuilder.addNextIntent(notificationDetailIntent);

                    PendingIntent resultPendingIntent = stackBuilder.getPendingIntent(0, PendingIntent.FLAG_UPDATE_CURRENT);
                    mBuilder.setContentIntent(resultPendingIntent);
                    NotificationManager mNotficationManager = (NotificationManager) getContext().getSystemService(Context.NOTIFICATION_SERVICE);
                    mNotficationManager.notify(WEATHER_NOTIFICATION_ID, mBuilder.build());
